...
public class ListsDataSource {
	...
	public void saveList(List_mirakle list) {
		Log.v(TAG, "saveTask");
		list.setSync_state(list.getSync_state()==Mirakel.SYNC_STATE_ADD?Mirakel.SYNC_STATE_ADD:Mirakel.SYNC_STATE_NEED_SYNC);		
	...
	protected void delete_list(final List_mirakle list,final String email,final String password,final String url) {
		new Network(new DataDownloadCommand() {
			@Override
			public void after_exec(String result) {
				// Do Nothing
		...
		},email,password,Mirakel.Http_Mode.DELETE).execute(url+"/lists/"+list.getId()+".json");
	...
	protected void sync_list(List_mirakle list, String email, String password,
		...
		}, email, password, Mirakel.Http_Mode.PUT,data).execute(url+"/lists/"+list.getId()+".json");	
	}

	protected void add_list(final List_mirakle list,final String email,final String password, final String url) {
		List<BasicNameValuePair> data=new ArrayList<BasicNameValuePair>();
		data.add(new BasicNameValuePair("list[name]", list.getName()));
		new Network(new DataDownloadCommand() {
			...
			public void after_exec(String result) {
				...
				database.update("tasks", values, "list_id="+list.getId() ,null);
				
			}
		},email,password,Mirakel.Http_Mode.POST,data).execute(url+"/lists.json");
	...
	protected void merge_with_server(List_mirakle[] lists_server) {
		for(int i=0;i<lists_server.length;i++){
			...
			if(list==null){
				long id=database.insert("lists", null, lists_server[i].getContentValues());
				ContentValues values=new ContentValues();
