...
public class CalDavSync {
	...
	public void sync(Account account) {
		...
		}, HttpMode.REPORT, content, ctx, account.name, AccountManager.get(ctx)
				.getPassword(account));
		n.execute(url);
	...
	private void syncToServer(List<Task> tasks, String url) {
		...
		for (Task t : tasks) {
			if (t.getSyncState() == SYNC_STATE.ADD
					|| t.getSyncState() == SYNC_STATE.IS_SYNCED) {
				// Add Taskcontent
				content += parseTask(t);
			} else {
		...
		}, HttpMode.PUT, content, ctx, account.name, AccountManager.get(ctx)
				.getPassword(account));
		n.execute(url);
	}
	...
	private String parseTask(Task t) {
		...
		if (t.getReminder() != null) {
			ret += "BEGIN:VALARM\n";
			ret += "TRIGGER;VALUE=DATE-TIME:"
					+ DateTimeHelper.formatTaskWarrior(t.getReminder()) + "\n";
			ret += "END:VALARM\n";
		}
	...
	private Task parseVTODO(String s) {
		...
		for (String l : lines) {
			if (l.contains("SUMMARY")) {
				...
				try {
					t.setCreatedAt(DateTimeHelper.parseCalDav(l.replace(
							"CREATED:", "")));
				...
				try {
					t.setUpdatedAt(DateTimeHelper.parseCalDav(l.replace(
							"LAST-MODIFIED:", "")));
				} catch (ParseException e) {
				...
				try {
					t.setDue(DateTimeHelper.parseCalDavDue(l.replace(
							"DUE;VALUE=DATE:", "")));
				} catch (ParseException e) {
				...
				try {
					t.setReminder(DateTimeHelper.parseTaskWarrior(l.replace(
