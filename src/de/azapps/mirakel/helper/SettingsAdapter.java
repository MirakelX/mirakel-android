...
public class SettingsAdapter extends ArrayAdapter<Header> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(header.id ==R.id.sync_header){
			view = mInflater.inflate(R.layout.preferences_switch, parent, false);
			...
			s.setChecked(PreferenceManager.getDefaultSharedPreferences(ctx).getBoolean("syncUse", false));
			s.setOnCheckedChangeListener(new OnCheckedChangeListener() {
				
				@Override
				public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					PreferencesHelper.createAuthActivity(isChecked, (Activity) ctx,s,false);
				}
			});
		}else{
			view = mInflater.inflate(R.layout.preferences_header_item, parent, false);
			((ImageView) view.findViewById(android.R.id.icon)).setImageResource(header.iconRes);
			((TextView) view.findViewById(android.R.id.title)).setText(header.getTitle(getContext()
					.getResources()));
			((TextView) view.findViewById(android.R.id.summary)).setText(header
