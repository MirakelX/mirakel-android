...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
    ...
    private static final ExceptionHandlerInitializer NULL_EXCEPTION_HANDLER_INITIALIZER = new
    ...
    ErrorReporter(Application context, SharedPreferences prefs, boolean enabled) {
        ...
        crashReportDataFactory = new CrashReportDataFactory(mContext, prefs, appStartDate,
    ...
    private void endApplication(Thread uncaughtExceptionThread, Throwable th) {
        ...
        final boolean letDefaultHandlerEndApplication = (
                    ...
                    (ACRA.getConfig().mode() == ReportingInteractionMode.TOAST &&
    ...
    public void setEnabled(boolean enabled) {
        Log.i(ACRA.LOG_TAG, "ACRA is " + (enabled ? "enabled" : "disabled") + " for " +
    ...
    SendWorker startSendingReports(boolean onlySendSilentReports, boolean approveReportsFirst) {
        final SendWorker worker = new SendWorker(mContext, mReportSenders, onlySendSilentReports,
                approveReportsFirst);
        worker.start();
        return worker;
    }
    ...
    private void report(final ReportBuilder reportBuilder) {
        ...
        final CrashReportData crashReportData = crashReportDataFactory.createCrashData(
                reportBuilder.mMessage,
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if (toastWaitEnded || (worker == null)) {
                    ...
                    Log.d(LOG_TAG, "Waiting for " + (toastWaitEnded ? "Toast " : " -- ") +
                          (worker.isAlive() ? "and Worker" : ""));
                ...
                Log.d(LOG_TAG, "Wait for Toast + worker ended. Kill Application ? " +
                      reportBuilder.mEndsApplication);
    ...
    private Intent createCrashReportDialogIntent(String reportFileName, ReportBuilder reportBuilder) {
        Log.d(LOG_TAG, "Creating DialogIntent for " + reportFileName + " exception=" +
    ...
    private void createNotification(String reportFileName, ReportBuilder reportBuilder) {
        ...
        final NotificationManager notificationManager = (NotificationManager) mContext.getSystemService(
        ...
        final PendingIntent contentIntent = PendingIntent.getActivity(mContext, mNotificationCounter++,
                                            crashReportDialogIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    private void deletePendingReports(boolean deleteApprovedReports, boolean deleteNonApprovedReports,
        ...
        for (int iFile = 0; iFile < filesList.length - nbOfLatestToKeep; iFile++) {
            ...
            if ((isReportApproved && deleteApprovedReports) || (!isReportApproved &&
                    deleteNonApprovedReports)) {
